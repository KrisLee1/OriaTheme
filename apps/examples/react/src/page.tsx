...
export function DemoPage({ framework }: { readonly framework: string }) {
  ...
  </main>{editorShown ? <aside className="editor-panel editor-sidebar" data-state={editorVisibility} id="theme-editor-panel" onAnimationEnd={event => { if (editorVisibility === "closing" && event.animationName === "editor-sidebar-dismiss") setEditorVisibility("closed"); }}><Suspense fallback={<div className="editor-loading" role="status">Loading theme editor…</div>}><ThemeEditor key={activeTheme.id} options={editorOptions} runtime={runtime} mode={snapshot.resolvedMode} onModeChange={(mode, origin) => changeAppearance(mode, origin)} previewFollowsAppearance onDirtyChange={setEditorDirty} {...(discardRequest ? { discardRequest } : {})} onClose={closeEditor} onSave={result => { if (result.ok) setTheme(result.theme.id); }} /></Suspense></aside> : null}</div>;
}
