...
export function ColorField(props: TokenFieldProps): ReactElement {
  ...
  return <FieldFrame props={props}><div data-oria-editor-color>
    <button type="button" data-oria-editor-color-swatch aria-label={`Choose ${props.field.label}`} style={{ "--oria-editor-color-preview": preview } as CSSProperties}><input tabIndex={-1} type="color" value={native} onChange={event => buffer.setText(event.target.value)} /></button>
