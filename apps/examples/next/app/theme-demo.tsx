...
import { ThemeTokenShowcase } from "./token-showcase";
...
export function ThemeDemo({ framework }: { readonly framework: string }) {
  ...
  const closeEditor = (): void => setEditorVisibility("closing");
  useEffect(() => {
  ...
  </main>{editorShown ? <aside className="editor-panel editor-sidebar" data-state={editorVisibility} id="theme-editor-panel" onAnimationEnd={event => { if (editorVisibility === "closing" && event.animationName === "editor-sidebar-dismiss") setEditorVisibility("closed"); }}><ThemeEditor key={activeTheme.id} options={editorOptions} runtime={runtime} mode={snapshot.resolvedMode} onModeChange={(mode, origin) => changeAppearance(mode, origin)} previewFollowsAppearance onDirtyChange={setEditorDirty} {...(discardRequest ? { discardRequest } : {})} onClose={closeEditor} onSave={result => { if (result.ok) setTheme(result.theme.id); }} /></aside> : null}</div>;
}
