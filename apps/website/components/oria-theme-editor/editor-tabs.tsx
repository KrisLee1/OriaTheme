...
export function EditorTabs({ tabs, active, onChange, issues }: { readonly tabs: readonly EditorTabLayout[]; readonly active: EditorTabId; readonly onChange: (tab: EditorTabId) => void; readonly issues: readonly ValidationIssue[] }): ReactElement {
  ...
  return <nav data-oria-editor-tabs role="tablist" aria-label={copy.themeCategories}>{tabs.map((tab, index) => {
    ...
    return <button type="button" role="tab" id={`oria-tab-${tab.id}`} aria-selected={active === tab.id} tabIndex={active === tab.id ? 0 : -1} key={tab.id} onClick={() => onChange(tab.id)} onKeyDown={event => move(event, index)}>{copy.tabs[tab.id]}{count ? <span aria-label={`${count} ${copy.issues}`}>{count}</span> : null}</button>;
  })}</nav>;
}
