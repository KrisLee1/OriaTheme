...
function layerSummary(layer: PatternLayer, copy: ReturnType<typeof useEditorCopy>["chrome"]["pattern"]): string { return layer.type === "noise" ? `${copy.noise[layer.variant]} · ${Math.round(layer.intensity * 100)}%` : `${copy.types[layer.type]} · ${layer.type === "dot" ? layer.angle ?? 0 : layer.angle}°`; }
...
function Icon({ kind }: { readonly kind: "up" | "down" | "delete" | "unset" }): ReactElement {
  const Glyph = kind === "up" ? ArrowUp : kind === "down" ? ArrowDown : kind === "delete" ? Trash2 : Ban;
...
function PatternLayerEditor({ id, layer, index, count, onChange, onMove, onDelete }: {
  ...
  const copy = useEditorCopy().chrome.pattern;
  ...
  return <fieldset data-oria-editor-pattern-layer>
    <legend className="oria-editor-visually-hidden">{interpolate(copy.layer, { index: index + 1 })}</legend>
    <header>
      <span><strong>{interpolate(copy.layer, { index: index + 1 })}</strong><small>{layerSummary(layer, copy)}</small></span>
      <label data-oria-editor-pattern-type><span className="oria-editor-visually-hidden">{interpolate(copy.layerType, { index: index + 1 })}</span><EditorSelect aria-label={interpolate(copy.layerType, { index: index + 1 })} value={layer.type} onChange={event => changeType(event.target.value as PatternType)}>{types.map(type => <option key={type} value={type}>{copy.types[type]}</option>)}</EditorSelect></label>
      <div data-oria-editor-pattern-layer-actions>
        <button type="button" aria-label={interpolate(copy.moveUp, { index: index + 1 })} title={copy.moveUpTitle} disabled={index === 0} onClick={() => onMove(-1)}><Icon kind="up" /></button>
        <button type="button" aria-label={interpolate(copy.moveDown, { index: index + 1 })} title={copy.moveDownTitle} disabled={index === count - 1} onClick={() => onMove(1)}><Icon kind="down" /></button>
        <button type="button" aria-label={interpolate(copy.delete, { index: index + 1 })} title={copy.deleteTitle} onClick={onDelete}><Icon kind="delete" /></button>
      </div>
    </header>
    <div data-oria-editor-pattern-layer-fields>
      <div data-oria-editor-color>
        <button type="button" data-oria-editor-color-swatch aria-label={interpolate(copy.chooseColor, { index: index + 1 })} style={{ "--oria-editor-color-preview": previewColor(colorBuffer.text) } as CSSProperties}><input tabIndex={-1} type="color" value={nativeColor(colorBuffer.text)} onChange={event => colorBuffer.setText(event.target.value)} /></button>
        <input value={colorBuffer.text} aria-label={interpolate(copy.color, { index: index + 1 })} aria-invalid={!safeColor(colorBuffer.text) || undefined} onChange={event => colorBuffer.setText(event.target.value)} spellCheck={false} />
        <BaseColorPalette id={`${id}-layer-${index}-base-colors`} label={interpolate(copy.color, { index: index + 1 })} value={colorBuffer.text} onSelect={colorBuffer.setText} />
      </div>
      {layer.type === "noise" ? <>
        <label><span>{copy.style}</span><EditorSelect aria-label={`${interpolate(copy.layer, { index: index + 1 })} ${copy.style}`} value={layer.variant} onChange={event => commit({ ...layer, variant: event.target.value as NoiseVariant })}>{noiseVariants.map(variant => <option key={variant} value={variant}>{copy.noise[variant]}</option>)}</EditorSelect></label>
        <DimensionParam label={copy.grainSize} ariaLabel={`${interpolate(copy.layer, { index: index + 1 })} ${copy.grainSize}`} value={layer.tileSize} onCommit={next => commit({ ...layer, tileSize: next })} />
        <NumberParam label={copy.intensity} ariaLabel={`${interpolate(copy.layer, { index: index + 1 })} ${copy.intensity}`} value={layer.intensity} minimum={0} maximum={1} step={0.01} valid={validIntensity} onCommit={next => commit({ ...layer, intensity: next })} />
      </> : <>
        <DimensionParam label={layer.type === "dot" ? copy.radius : layer.type === "stripe" ? copy.stripeWidth : copy.lineWidth} ariaLabel={`${interpolate(copy.layer, { index: index + 1 })} ${layer.type === "dot" ? copy.radius : layer.type === "stripe" ? copy.stripeWidth : copy.lineWidth}`} value={layer.type === "dot" ? layer.radius : layer.type === "stripe" ? layer.stripeWidth : layer.lineWidth} onCommit={next => commit(layer.type === "dot" ? { ...layer, radius: next } : layer.type === "stripe" ? { ...layer, stripeWidth: next } : { ...layer, lineWidth: next })} />
        <DimensionParam label={copy.spacing} ariaLabel={`${interpolate(copy.layer, { index: index + 1 })} ${copy.spacing}`} value={layer.spacing} onCommit={next => commit({ ...layer, spacing: next })} />
        <NumberParam label={copy.angle} ariaLabel={`${interpolate(copy.layer, { index: index + 1 })} ${copy.angle}`} value={layer.type === "dot" ? layer.angle ?? 0 : layer.angle} minimum={0} maximum={360} step={1} valid={validAngle} onCommit={next => commit({ ...layer, angle: next })} />
      </>}
    </div>
  </fieldset>;
}
...
export function PatternField(props: TokenFieldProps): ReactElement {
  ...
  const patternName = props.field.label;
  ...
  if (!layers) return <FieldFrame props={props}><div data-oria-editor-pattern data-empty><div data-oria-editor-pattern-preview style={{ background: preview([defaults("dot")]) }}><span>{interpolate(copy.notSet, { name: patternName })}</span></div><button type="button" data-oria-editor-pattern-create onClick={() => commit([defaults("dot")])}>{interpolate(copy.create, { name: patternName })}</button></div></FieldFrame>;
  ...
  return <FieldFrame props={props}><div data-oria-editor-pattern>
    <div data-oria-editor-pattern-preview style={{ background: preview(layers) }} role="img" aria-label={interpolate(copy.layersPreview, { count: layers.length, name: patternName })}><span>{interpolate(copy.layersOnTop, { count: layers.length, layers: layers.length === 1 ? copy.layerOne : copy.layerMany })}</span></div>
    ...
    <div data-oria-editor-pattern-footer><label data-oria-editor-pattern-add><span>{copy.addLayer}</span><EditorSelect aria-label={copy.addLayerLabel} value="" disabled={layers.length >= 8} onChange={event => { const type = event.target.value as PatternType; if (types.includes(type)) commit([...layers, defaults(type)]); }}><option value="">{layers.length >= 8 ? copy.maximum : copy.choose}</option>{types.map(type => <option key={type} value={type}>{copy.types[type]}</option>)}</EditorSelect></label><button type="button" data-oria-editor-pattern-unset aria-label={interpolate(copy.unsetLabel, { label: props.field.label })} title={interpolate(copy.unsetTitle, { name: patternName })} onClick={() => props.session.removeToken(props.mode, props.field.path)}><Icon kind="unset" />{copy.unset}</button></div>
  </div></FieldFrame>;
}
