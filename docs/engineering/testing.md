...
- 任务报告或里程碑证据记录实际命令、pass/fail、测试数量和失败摘要；普通任务不因执行测试而自动追加日志。
- “未执行”不能写为通过。
- 修复缺陷必须先添加或同步添加回归测试。
- Fast 运行最小定向检查，Review 覆盖受影响数据流，Controlled 按批准提案验证；Phase/发布验收执行完整命令。
- 旧日志或旧状态中的通过结果只能作为历史基线，不能替代本次受影响范围的检查。
