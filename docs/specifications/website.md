...
官网位于 `apps/website`，是 workspace 中的私有应用，`package.json` 必须设置 `private: true`，不得加入 Changesets 发布清单或 npm pack 流程。现有 `apps/docs` 的内容在 Phase 8 迁入官网后移除重复入口。
...
- 预设缩略图和编辑器等重内容延迟加载；具体性能预算在 Phase 8 实现开始时基于选定部署环境固化。
...
- `TBD-REG-01`：官方组件 registry 基地址确定为 `https://theme.oria.org.cn/registry/v1`。托管平台选定后必须部署静态 manifest 与模板、配置 TLS，并在仓库外干净项目完成 HTTPS `add` / `diff` 验证；在此之前仍阻塞公开 CLI/registry、首发与官网启动，但不阻塞本地 registry/CLI 实现。
