...
`theme tailwind-bridge` 复用 `@oriatheme/tailwind` 生成器，预构建静态 Tailwind v4 bridge（CSS 变量名不能在运行时拼接，custom prefix 必须提前生成）：

```bash
pnpm dlx @oriatheme/cli theme tailwind-bridge --prefix acme --out src/oria-tailwind.css
```

- `--prefix` 可选，默认 `oria`；只允许字母开头、由字母/数字/连字符组成的值，非法值直接拒绝。
- `--out` 必填，必须是项目内的规范化相对路径；已存在文件只在显式 `--overwrite` 时覆盖，`--dry-run` 只显示计划。
- 该命令只写入目标 CSS 文件，不修改 `package.json`、lockfile 或 `.oria` 元数据，也不运行包管理器。
